...
router.post('/', async (req, res) => {
    ...
    if (!client_id || !bag_count || !waste_type || latitude == null || longitude == null || !payment_method) {
    ...
    if (!Number.isInteger(bag_count) || bag_count <= 0) {
        return res.status(400).json({ error: 'bag_count must be a positive integer' });
    }
    ...
    try {
        const clientResult = await pool.query('SELECT * FROM find_client_by_id($1)', [client_id]);
        ...
        const { mobilityTypeName, nearestDumpsterId, distanceMeters } =
            await evaluateMobility(bag_count, latitude, longitude);
        ...
        const estimatedPriceFcfa = bag_count * 500;
        ...
        const created = await withTenant(client.company_id, async (dbClient) => {
            const result = await dbClient.query(
                ...
                [client_id, bag_count, waste_type, mobilityTypeId, initialStatus, adminHoldExpiresAt, latitude, longitude, payment_method, estimatedPriceFcfa]
            );
        ...
        const updated = await withTenant(company_id, async (client) => {
            const result = await client.query(
                `UPDATE pickup_requests
                 ...
                 WHERE id = $2 AND routing_status = 'searching_corporate' AND collector_id IS NULL
                 RETURNING id, routing_status, collector_id`,
                [collector_id, requestId]
            );
            return result.rows[0];
        });
...
router.get('/:id', async (req, res) => {
    const requestId = Number(req.params.id);
    try {
        const result = await pool.query('SELECT * FROM get_pickup_request_status($1)', [requestId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'pickup request not found' });
        }
        return res.json(result.rows[0]);
